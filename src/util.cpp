...
const char * const ELETACOIN_CONF_FILENAME = "eletacoin.conf";
const char * const ELETACOIN_PID_FILENAME = "eletacoin.pid";
...
#else
    const char* pszModule = "eletacoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EletaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EletaCoin
    // Mac: ~/Library/Application Support/EletaCoin
    // Unix: ~/.eletacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EletaCoin4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EletaCoin4";
#else
    // Unix
    return pathRet / ".eletacoin4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", ELETACOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No eletacoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override eletacoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", ELETACOIN_PID_FILENAME));
