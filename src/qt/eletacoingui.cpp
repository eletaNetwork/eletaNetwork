...
const std::string EletaCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString EletaCoinGUI::DEFAULT_WALLET = "~Default";

EletaCoinGUI::EletaCoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void EletaCoinGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/eletacoin"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a EletaCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and eletacoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your EletaCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified EletaCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a eletacoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible EletaCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void EletaCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(EletaCoinUnits::Unit u, EletaCoinUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(EletaCoinUnits::name(u)), this);
...
void EletaCoinGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void EletaCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        topMenuLogo->setObjectName("eletaLogo");
        topMenuLogo->move(0,0);
        topMenuLogo->setStyleSheet(
           "#eletaLogo { border-image: url(:/icons/menu_logo)  0 0 0 0 stretch stretch; border: 0px; }");
        ...
        // ImageButton* eletaLogo2 = new ImageButton();
        // eletaLogo2->setFixedSize(64,64);
        // eletaLogo2->setObjectName("eletaLogo");
        // eletaLogo2->setPixmap(pixmap);
        //
        // toolbar->setMovable(false);
        // toolbar->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        // toolbar->addWidget(eletaLogo);
        // toolbar->addWidget(eletaLogo2);
...
void EletaCoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool EletaCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void EletaCoinGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool EletaCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void EletaCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void EletaCoinGUI::repairWallet()
{
...
void EletaCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void EletaCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void EletaCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void EletaCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void EletaCoinGUI::optionsClicked()
{
...
void EletaCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void EletaCoinGUI::showDebugWindow()
{
...
void EletaCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void EletaCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void EletaCoinGUI::openClicked()
{
...
void EletaCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void EletaCoinGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void EletaCoinGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void EletaCoinGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void EletaCoinGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void EletaCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void EletaCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void EletaCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to EletaCoin network", "", count));
    if(walletFrame){
...
void EletaCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to EletaCoin network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to EletaCoin network..."));
...
void EletaCoinGUI::showVotingDialog()
{
...
void EletaCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("EletaCoin"); // default title
...
void EletaCoinGUI::changeEvent(QEvent *e)
{
...
void EletaCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void EletaCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void EletaCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(EletaCoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void EletaCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void EletaCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool EletaCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool EletaCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void EletaCoinGUI::setEncryptionStatus(int status)
{
...
void EletaCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void EletaCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void EletaCoinGUI::detectShutdown()
{
...
void EletaCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void EletaCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(EletaCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void EletaCoinGUI::subscribeToCoreSignals()
{
...
void EletaCoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<EletaCoinUnits::Unit> units = EletaCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const EletaCoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(EletaCoinUnits::name(unit)));
    }
...
{
    setText(EletaCoinUnits::name(newUnits));
}
...
void EletaCoinGUI::toggleStaking()
{
...
void EletaCoinGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void EletaCoinGUI::updatePrice()
{
  ...
  request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/eleta-coin/?convert=EUR"));
...
void EletaCoinGUI::replyFinished(QNetworkReply *reply)
{
...
void EletaCoinGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CEletaCoinAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void EletaCoinGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.eletacoin.org/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void EletaCoinGUI::updateStakingStatus()
{
    updateWeight();
