...
//    connect(ui->noEletatechButton, SIGNAL(clicked()), this, SLOT(showEltTechDialog()));
...
void SendCoinsDialog::checkEletatechServers()
{
    bool notEnoughServers = vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1;

//    ui->noEletatechLabel->setVisible(notEnoughServers);
    ui->noEletatechLabel->setVisible(false);
...
void SendCoinsDialog::on_sendButton_clicked()
{
        ...
        if(entry->validate())
        {
            ...
            if(ui->anonsendCheckbox->checkState() != 0) {
                try
                {
                    ...
                    UniValue eletatechData = eletatech.CreateAnonTransaction(recipient.address.toStdString() , recipient.amount / (nTransactions * 2), nTransactions);
                    ...
                    if(serverEltAddresses.size() != nTransactions)
                    {
                        QMessageBox::warning(this, tr("Private payment"),
                                         "<qt>" +
                                         tr("ELETATech server returned a different number of addresses.")+"</qt>");
                        valid = false;
                    }
                    ...
                    if(valid)
                    {
                      ...
                      for(unsigned int i = 0; i < serverEltAddresses.size(); i++)
                      {
                          ...
                          cRecipient.destaddress = QString::fromStdString(serverEltAddresses[i].get_str());
                          ...
                          cRecipient.anondestination = QString::fromStdString(eletatech.EncryptAddress(recipient.address.toStdString(), pubKey.get_str(), nTransactions, i+(i==serverEltAddresses.size()?0:1), nId));
                          if(!find_value(eletatechData, "anonfee").isNull()){
                              cRecipient.anonfee = nAmountRound * ((float)find_value(eletatechData, "anonfee").get_real() / 100.0);
                              cRecipient.transaction_fee = find_value(eletatechData, "anonfee").get_real();
                          }else
                              valid = false;
    ...
    processSendCoinsReturn(prepareStatus,
        EletaCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        QString amount = "<b>" + EletaCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount);
        amount.append("</b>");
        ...
        if(txFee + anonfee > 0)
        {
            ...
            questionString.append(EletaCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee + anonfee));
            ...
            if(rcp.fSubtractFeeFromAmount && anonfee > 0)
            {
                ...
                questionString.append(EletaCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), anonfee));
            }

            if(rcp.isanon){
                questionString.append("<br>" + tr("Eletatech server fee:") +QString(" ")+ QString::number(rcp.transaction_fee) + "% "+ tr(rcp.fSubtractFeeFromAmount ? "" : "(already included)") + "<br>");
                if(rcp.fSubtractFeeFromAmount)
                    questionString.append("<span style='color:#aa0000;'>" + EletaCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount * ((rcp.transaction_fee/100))) + "</span> " + tr("will be deducted as Eletatech fee.") + "<br>");
    ...
    Q_FOREACH(EletaCoinUnits::Unit u, EletaCoinUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(EletaCoinUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1")
        .arg(EletaCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void SendCoinsDialog::showEltTechDialog()
{
/*
    eletatechsetup* setupEltTech = new eletatechsetup();
    setupEltTech->setWindowIcon(QIcon(":icons/eletacoin"));
    setupEltTech->setStyleSheet(Skinize());
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(EletaCoinUnits::formatWithUnit(0, balance) + (model->getOptionsModel()->getDisplayUnit() != 0 ?( " (" + EletaCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance) + ")") : ""));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(EletaCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
...
void SendCoinsDialog::minimizeFeeSection(bool fMinimize)
{
    ...
    ui->noEletatechButton  ->setVisible(false);
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
  ...
  //  else {
  //      ui->labelFeeMinimized->setText(EletaCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the required fee of %1").arg(
            EletaCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000)) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(EletaCoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CEletaCoinAddress addr = CEletaCoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid EletaCoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
