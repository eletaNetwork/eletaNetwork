...
bool CFund::IsValidPaymentRequest(CTransaction tx)
{
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nAmount) + "ELT from the proposal " + proposal.hash.ToString() + ". Payment request id: " + strDZeel;

    CEletaCoinAddress addr(proposal.Address);
    CKeyID keyID;
    addr.GetKeyID(keyID);
