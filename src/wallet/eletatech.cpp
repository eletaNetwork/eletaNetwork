...
UniValue Eletatech::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    UniValue addresses = find_value(serverData, "eleta_addresses");
    UniValue addrArray = addresses.get_array();

    UniValue eletatechData;
    eletatechData.setObject();
    eletatechData.pushKV("anondestination", encryptedAddress);
    eletatechData.pushKV("public_key", pubKey);
    eletatechData.pushKV("anonaddress", addrArray);
    eletatechData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    eletatechData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return eletatechData;
  } else {
    throw runtime_error("Unable to send ELETATech transaction, please try again.");
  }

}

vector<anonServer> Eletatech::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one ELETATech server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid ELETATech server to send private payments.");
      return returnServers;
  }
...
UniValue Eletatech::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured ELETATech nodes are available right now.");
  }
...
UniValue Eletatech::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string Eletatech::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",ELETATECH_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int Eletatech::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * Eletatech::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool Eletatech::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue Eletatech::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue eletatechData;
        eletatechData.setObject();
        eletatechData.pushKV("min_amount", find_value(data, "min_amount"));
        eletatechData.pushKV("max_amount", find_value(data, "max_amount"));
        eletatechData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return eletatechData;
      }
