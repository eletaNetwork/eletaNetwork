...
The RPM spec file provided here is for EletaCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://eletacoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[EletaCoin git repository](https://github.com/eletacoin/eletacoin) but are not part of
...
Currently only `Source10` is used, for the example `eletacoin.conf` file.
...
in the eletacoin git repository. At present only an SVG version of the EletaCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the eletacoin
...
needed when 0.12.1 is released, a proper fix is already in the EletaCoin git
master branch.
...
idea to build EletaCoin Core against a static version of that library compiled
...
The `eletacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `eletacoin-server` package. At this point, most RPM based
...
The `eletacoin-server` package should have SELinux support. How to properly do
...
EletaCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/eletacoin/). He has been
packaging EletaCoin for Fedora at least since 2012.
...
4. I renamed the `eletacoin` package that contains the Qt GUI to `eletacoin-core` as
that appears to be how the general population refers to it, in contrast to
`eletacoin-xt` or `eletacoin-classic`. I wanted to make sure the general population
