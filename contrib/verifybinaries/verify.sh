...
###   This script attempts to download the signature file SHA256SUMS.asc from eletacoin.org
...
WORKINGDIR="/tmp/eletacoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://eletacoin.org/bin/"
VERSIONPREFIX="eletacoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'eletacoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the EletaCoin Core binary release signing key installed?"
   fi
